...
export function clusterFileUpload(evt: Event, table: Handsontable, myChart: Chart<'line'>) {
    ...
    reader.onload = () => {
        ...
        updateScatter(table, myChart,
            document.getElementById('cluster-form') as ClusterForm, 1,
...
function updateScatter(table: Handsontable, myChart: Chart, clusterForm: ClusterForm, dataSetIndex: number, filterForm: FilterForm, err = 1) {
    ...
    let blueErr = columns.indexOf(filterForm["blue"].value + "err")<0? null:columns.indexOf(filterForm["blue"].value + "err"); //checks for supplied err data
    let redErr  = columns.indexOf(filterForm["red"].value + "err")<0? null:columns.indexOf(filterForm["red"].value + "err");
    let lumErr  = columns.indexOf(filterForm["lum"].value + "err")<0? null:columns.indexOf(filterForm["lum"].value + "err");
